...
static bool SetStartOnBoot(bool enable) {
    ...
    bool ok;
    if (enable) {
        // Get current EXE path and append --autostart argument
        wchar_t exePath[MAX_PATH];
        GetModuleFileNameW(nullptr, exePath, MAX_PATH);
        std::wstring cmdList = std::wstring(L"\"") + std::wstring(exePath) + L"\" --autostart";
        ok = (RegSetValueExW(hKey, kAppName, 0, REG_SZ,
                             reinterpret_cast<const BYTE*>(cmdList.c_str()),
                             static_cast<DWORD>((cmdList.length() + 1) * sizeof(wchar_t)))
              == ERROR_SUCCESS);
    } else {
...
// ─────────────────────────────────────────────
static void RegisterMethodHandlers(flutter::BinaryMessenger* messenger, HWND window_handle) {
    ...
    channel->SetMethodCallHandler(
        [window_handle](const flutter::MethodCall<flutter::EncodableValue>& call,
           std::unique_ptr<flutter::MethodResult<flutter::EncodableValue>> result) {
            ...
            } else if (call.method_name() == "startClipboardListener") {
                // Ensure flutter is attached before listening
                bool success = AddClipboardFormatListener(window_handle) != 0;
                if (success) result->Success(flutter::EncodableValue(true));
                else result->Error("ERR", "Failed to AddClipboardFormatListener");
